...
const generateAccessToken = async () => {
  console.log('[Adobe PDF API] generating access token...');
...
const generatePresignedURI = async (token, type) => {
  console.log('[Adobe PDF API] generating presigned URI...');
...
const startExtractJob = async (token, assetID) => {
  console.log('[Adobe PDF API] attempting text extraction...');
  ...
  try {
    ...
    console.log('[Adobe PDF API] polling for job completion...');
    let jobStatus;
    let result;
    do {
      ...
      console.log('[Adobe PDF API] Current job status: ', jobStatus);
    ...
    if (jobStatus === 'done') {
      ...
      console.log('[Adobe PDF API] Text extraction failed :(');
      console.log('[Adobe PDF API] Code: ', result.data.error.code);
      console.log('[Adobe PDF API] Message: ', result.data.error.message);
      return 500;
    }
  } catch (error) {
    console.log('Error: ', error);
...
const retrieveDocxTemplate = async (template) => {
  console.log('[Adobe PDF API] retrieving doc template...');
  try {
    const file = await fetch(template);

    const arrayBuffer = await file.arrayBuffer();

    const docxTemplate = new Blob([new Uint8Array(arrayBuffer)], {
...
const startDocGeneration = async (token, assetID, jsonData) => {
  console.log('[Adobe PDF API] merging data with quiz template...');
  ...
  try {
    ...
    if (jobStatus === 'done') {
      ...
      console.log('[Adobe PDF API] Document generation failed :(');
      console.log('[Adobe PDF API] Code: ', result.data.error.code);
      console.log('[Adobe PDF API] Message: ', result.data.error.message);
    }
  } catch (error) {
  ...
  try {
    ...
    if (uploadSuccessful === 200) {
      ...
      if (jobStatus === 'done') {
        ...
        console.log('[Adobe PDF API] Document encryption failed :(');
        console.log('[Adobe PDF API] Code: ', result.data.error.code);
        console.log('[Adobe PDF API] Message: ', result.data.error.message);
      }
    }
...
export const downloadQuiz = async (
  ...
  if (uploadSuccessful === 200) {
    ...
    if (pwd) {
      console.log('[Adobe PDF API] Encrypting file...');
